...
namespace EasyAdmin
{
    static public class Tools
    {
        ...
        public static void FillDataTableNullValues(DataTable changes)
        {
            int rcount = changes.Rows.Count;
            int ccount = changes.Columns.Count;
            for (int r = 0; r < rcount; r++)
            {
                for (int c = 0; c < ccount; c++)
                {
                    //object dv = changes.Columns[c].DefaultValue;
                    if (changes.Rows[r].RowState == DataRowState.Deleted)
                        continue;
                    if (changes.Rows[r][c] == DBNull.Value /*&& !changes.Columns[c].AutoIncrement*/)
                    {
                        switch (changes.Columns[c].DataType.Name)
                        {
                            case "Boolean":
                                changes.Rows[r][c] = 0;
                                break;
                            ...
                            case "Int64":
                                changes.Rows[r][c] = 0;
                                break;
                            default: //Byte, Char, DatTime, Decimal, SByte, Single, TimeSpan NOT YET IMPLEMENTED
                                break;
                        }
                    }
                }
